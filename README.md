...
Run it now: notebooks/MedicaidGuard_XAI_Colab.ipynb runs the whole pipeline on Colab's free CPU tier in 10–20 minutes.
⸻
Research question
...
Open notebooks/MedicaidGuard_XAI_Colab.ipynb in Colab. It installs dependencies, runs the full pipeline, renders the curves and explanations, runs the tests, and includes a cell that pushes the repository to GitHub using a token entered through getpass so it is never written into the saved notebook.
⸻
API
...
No endpoint changes a claim's status, denies a service, or alters an authorisation. That is deliberate and documented in SECURITY.md.
⸻
...
See reports/limitations.md and docs/methodology.md.
⸻
Documentation
Document	Contents
reports/dataset_discovery.md	Full public-dataset search, evaluation, and selection rationale
reports/research_report.md	Research report built from the actual results
...
See CITATION.cff. ORCID: 0009-0002-0795-4512.
⸻
License
